...
SERVO_IDX_UPPER_ARM_R = 9 # Upper Arm Rotation Right (Brick 1)
SERVO_IDX_THUMB_R_OPPOSITION = 0
SERVO_IDX_THUMB_R_PROXIMAL = 1
SERVO_IDX_INDEX_R_PROXIMAL = 2
SERVO_IDX_MIDDLE_R_PROXIMAL = 3
SERVO_IDX_RING_R_PROXIMAL = 4
SERVO_IDX_PINKY_R_PROXIMAL = 5
# Brick 2 (Shoulder Vertical + Shoulder Horizontal)
SERVO_IDX_SHOULDER_VERTICAL_L = 9
SERVO_IDX_SHOULDER_VERTICAL_R = 1
SERVO_IDX_SHOULDER_HORIZONTAL_L = 0 # Actual Shoulder Horizontal Left (Brick 2)
SERVO_IDX_SHOULDER_HORIZONTAL_R = 8 # Actual Shoulder Horizontal Right (Brick 2)
# Brick 3 (Left Arm + Fingers)
SERVO_IDX_UPPER_ARM_L = 9 # Upper Arm Rotation Left (Brick 3)
...
class HandTracker:
    ...
    def next_frame(self):
        ...
        if len(res.get("lm_score",[])) == 0: # No hands detected, move to default/initial position
            ...
            # Shoulder Horizontal Left (Initial Position - Brick 2)
            set_servo(servoBrick2, SERVO_IDX_SHOULDER_HORIZONTAL_L, 0) # Set to desired initial position
            # Shoulder Horizontal Right (Initial Position - Brick 2)
            set_servo(servoBrick2, SERVO_IDX_SHOULDER_HORIZONTAL_R, 0) # Set to desired initial position
        ...
        if norm_landmarks:          # only calculate if hand coordinates are available
            ...
            if index_hand_right > -1: # If right hand is detected
                ...
                # Control left fingers based on calculated right hand finger angles (value_... variables)
                ...
                set_servo(servoBrick3, SERVO_IDX_INDEX_L_PROXIMAL, value_idx)
                set_servo(servoBrick3, SERVO_IDX_MIDDLE_L_PROXIMAL, value_mid)
                set_servo(servoBrick3, SERVO_IDX_RING_L_PROXIMAL, value_rng)
                set_servo(servoBrick3, SERVO_IDX_PINKY_L_PROXIMAL, value_ltl)
            ...
            if index_hand_left > -1: # If left hand is detected
                ...
                if left_hand_angle_col != -1 and angle_array.size > 0 and angle_array.shape[1] > left_hand_angle_col:
                    ...
                    # Clamp values to servo limits if necessary (e.g., 0 to 9000) - Add clamping if needed

                    # Control right fingers using servoBrick1 and right finger indices
                    set_servo(servoBrick1, SERVO_IDX_THUMB_R_OPPOSITION, value_thumb_stretch2_r)
                    set_servo(servoBrick1, SERVO_IDX_THUMB_R_PROXIMAL, value_thumb_stretch_r)
                    set_servo(servoBrick1, SERVO_IDX_INDEX_R_PROXIMAL, value_idx_r)
                    set_servo(servoBrick1, SERVO_IDX_MIDDLE_R_PROXIMAL, value_mid_r)
                    set_servo(servoBrick1, SERVO_IDX_RING_R_PROXIMAL, value_rng_r)
                    set_servo(servoBrick1, SERVO_IDX_PINKY_R_PROXIMAL, value_ltl_r)
